...
def main(opt, writer, metadata):
    ...
    if opt.backbone == 'resnet':
        optimizer = torch.optim.SGD(model.parameters(), lr=0.002, momentum=0.99, weight_decay=0.0005) # Be careful when you adjust learning rate, you can refer to the linear scaling rule
...
if __name__ == "__main__":
    ...
    root = Path(r"F:\pumpkinCode\Change Detection\result")
    save_path = root / opt.dataset / opt.backbone / opt.mode

    """
    Initialize experiments log
    """
    logging.basicConfig(level=logging.INFO)
    writer = SummaryWriter(save_path / (f'{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}'))
