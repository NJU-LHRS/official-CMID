...
class Attention(nn.Module):
    ...
    def forward_with_attn(self, x, H, W, rel_pos_bias=None):
        B, N, C = x.shape
        qkv_bias = None
...
class RotatedVariedSizeWindowAttention(nn.Module):
    ...
    def forward_with_attn(self, x, H, W):
        B, N, C = x.shape
        assert N == H * W
        x = x.view(B, H, W, C)
...
class NormalCell(nn.Module):
    ...
    def forward_with_attention(self, x, H, W):
        ...
        if self.class_token:
            ...
            idienty = x
            x = self.norm1(x)
            x, attn = self.attn.forward_with_attn(x, H, W)
            x = self.drop_path(x)
            x = x + idienty
            x = x + convX
...
class ViTAE_NC_Win_RVSA(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 use_abs_pos_emb=True, out_indices=[11], interval=3, pretrained=None,
    ...
    def load_pretrain_model(self, state_dict: Dict):
        target_state_dict = state_dict["model"]
        self.eval()
        state_dict = self.state_dict()
        # for k in ['head.weight', 'head.bias']:
        #     if k in state_dict and state_dict[k].shape != target_state_dict[k].shape:
        #         print(f"Removing key {k} from pretrained checkpoint")
        #         del target_state_dict[k]

        # interpolate position embedding
        interpolate_pos_embed(self, target_state_dict)
        ...
        msg = self.load_state_dict(target_state_dict, strict=False)
        print(msg)
    ...
    def get_selfattention(self, data: torch.Tensor, n: int = 1):
        x, hw_shape = self.patch_embed(data)
        if self.pos_embed is not None:
            x = x + self.pos_embed[:, 1:, :]
        x = self.pos_drop(x)
    ...
    def get_last_selfattention(self, x: torch.Tensor, hw_shape):
        for i, stage in enumerate(self.blocks):
            if i < len(self.blocks) - 1:
                ...
                x, attns = stage.forward_with_attention(x, *hw_shape)
                return attns
    ...
    def extract(self, data:torch.tensor, device: torch.device = torch.device("cpu")):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in out_indices:
                B, _, C = x.shape
                out = x.reshape(B, Hp, Wp, C).permute(0, 3, 1, 2).contiguous()
                features.append(out)
