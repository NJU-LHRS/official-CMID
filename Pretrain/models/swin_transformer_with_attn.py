...
from .swin_utils import (to_2tuple, resize_pos_embed)
...
class WindowMSA(BaseModule):
    ...
    def forward_with_attn(self, x, mask=None):
        ...
        attn_out = self.softmax(attn)

        attn = self.attn_drop(attn_out)
...
class ShiftWindowMSA(BaseModule):
    ...
    def forward_with_atten(self, query, hw_shape):
        B, L, C = query.shape
        H, W = hw_shape
        ...
        attn_windows, attn = self.w_msa.forward_with_attn(query_windows, mask=attn_mask)
...
class SwinBlock(BaseModule):
    ...
    def forward_with_attn(self, x, hw_shape):
        identity = x
        x = self.norm1(x)
        x, attn = self.attn.forward_with_atten(x, hw_shape)
        x = x + identity
...
class SwinBlockSequence(BaseModule):
    ...
    def forward_with_attention(self, x: torch.Tensor, hw_shape):
        attns = []
        for block in self.blocks:
            x, attn = block.forward_with_attn(x, hw_shape)
            attns.append(attn)
...
class SwinTransformer(BaseModule):
    ...
    def forward(self, x, extract: bool = False):
        ...
        for i, stage in enumerate(self.stages):
            ...
            if i in self.out_indices:
                norm_layer = getattr(self, f'norm{i}') if not extract else nn.Identity()
                out = norm_layer(out)
